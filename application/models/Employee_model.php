...
class Employee_model extends CI_Model {
	...
	public function save_data($data)	 
	  ...
	  function emp_validate($username,$password){
    	return $this->db->select('*')->from($this->table)->where('employee_id',$username)->where('password',$password)->get()->row_array();
           ...
           function emp_email_validate($username,$password){
    	return $this->db->select('*')->from($this->table)->where('email',$username)->where('password',$password)->get()->row_array();
       ...
       function count_emp($username,$password){
      return $this->db->select('*')->from($this->table)->where('employee_id',$username)->where('password',$password)->get()->num_rows();
	...
	$this->db->delete('table_employee');
	 ...
	 function fetch_employee_depart_name($depart_id)
		...
		public function getEmployeeDataByStoreName($status)
		 {
		  $this->db->select("*");
		  $this->db->from('table_employee');
		...
		public function show_employee_by_id($emp_srt_id=0,$department_id=0,$designation_id=0)
	    {
		       $this->db->select('*');
		       $this->db->from('table_employee');
		        if($emp_srt_id!='all'){
		        	 $this->db->where('emp_store_name',$emp_srt_id);
		        }
		         if($department_id!='all'){
		        	 $this->db->where('department',$department_id);
		        }
		  ...
		  public function get_underemp_data($empid)
		 {
		  ...
		  public function getEmpAllData()
            ...
            public function get_employee_by_empcode($id = 0)
            {
            if ($id === 0)
            {
            $query = $this->db->get('table_employee');
            ...
            $query = $this->db->get_where('table_employee', array('employee_id' => $id));
            return $query->row_array();
            }
